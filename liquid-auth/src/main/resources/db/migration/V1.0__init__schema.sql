...
(
    ...
    `web_server_redirect_uri` varchar(256) comment '客户端重定向URL，authorization_code和implicit模式时需要该值进行校验，例如:https://baidu.com',
    ...
    `auto_approve`             varchar(256) comment '默认false,适用于authorization_code模式,设置用户是否自动approval操作,设置true跳过用户确认授权操作页面，直接跳到redirect_uri'
);
#客户端Token信息表
...
values ('resourceclient', null, '{noop}123456', 'all,read,write', 'authorization_code,refresh_token,password',
        'https://www.baidu.com', 'role_trusted_client', 7200, 7200, null, 'true');

#生成接入方测试Client配置信息
insert intO `oauth_client_details`(`client_id`, `resource_ids`, `client_secret`, `scope`, `authorized_grant_types`,
                                   `web_server_redirect_uri`, `authorities`, `access_token_validity`,
                                   `refresh_token_validity`, `additional_information`, `auto_approve`)
VALUES ('accessDemo', NULL, '{noop}123456', 'all,read,write', 'authorization_code,refresh_token,password',
        'https://www.baidu.com', 'ROLE_TRUSTED_CLIENT', 7200, 7200, NULL, 'true');