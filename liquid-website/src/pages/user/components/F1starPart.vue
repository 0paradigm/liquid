...
export default {
  ...
  data() {
    return {
      ...
      languages: [
        ...
        {value: '2', content: 'python'},
        {value: '3', content: 'cpp'}
      ],

      sortBy: '1',
      sort: [
        {value: '1', content: 'Most stars'},
        {value: '2', content: 'Recently active'},
      ],
      repos: [],
    }
  },
  ...
  methods: {
    ...
    init() {
      request.get(BACKEND_URL + `/core/api/user/stars/${this.$route.params.user}`, {
        headers: {
          'Authorization': jsCookie.get('Authorization')
        }
      }).then(res => {
