<template>
  <div style="width: 950px; padding-top: 20px">
    <a-typography-title :heading="4">
      {{stars.length}} {{stars.length > 1?'peoples are' : 'people is'}} watching {{$route.params.user}}/{{$route.params.rep}}
    </a-typography-title>
    <a-divider/>
    <a-row :gutter="[24, 24]">
...
export default {
  ...
  mounted() {
    ...
    request.get(BACKEND_URL+`/core/api/repo/list_watchers/${this.$route.params.user}/${this.$route.params.rep}`)
        .then(resp => {
